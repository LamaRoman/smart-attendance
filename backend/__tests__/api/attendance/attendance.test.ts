// Payroll Calculation Tests for Express Backend
...
describe('Payroll Calculations', () => {
  ...
  describe('SSF (Social Security Fund) Calculations', () => {
    ...
    it('should handle salary below SSF threshold', () => {
      const lowSalary = 20000;
      const employeeSSF = lowSalary * 0.11;

      expect(employeeSSF).toBe(2200);
    });
  });
  ...
  describe('TDS (Tax Deduction at Source) Calculations', () => {
    ...
    it('should calculate TDS with progressive brackets', () => {
      ...
      expect(tds).toBe(60000); // 20,000 + 40,000
    });
    ...
    it('should handle high income tax bracket', () => {
      ...
      // Simplified calculation for bracket above 2 million
      let tds = 0;
      
      // 0-500k: exempt
      // 500k-700k: 10% on 200k = 20,000
      tds += 200000 * 0.10;
      
      // 700k-1m: 20% on 300k = 60,000
      tds += 300000 * 0.20;
      
      // 1m-2m: 30% on 1m = 300,000
      tds += 1000000 * 0.30;
      
      expect(tds).toBe(380000);
    });
  });
  ...
  describe('Attendance-based Calculations', () => {
    ...
    it('should calculate salary for half-month work', () => {
      ...
      expect(proratedSalary).toBeCloseTo(30000, 0);
    });
  });
  ...
  describe('Overtime Calculations', () => {
    ...
    it('should calculate night shift differential (25%)', () => {
      ...
      const nightDifferential = 0.25;
      
      const nightShiftPay = hourlyRate * nightShiftHours * (1 + nightDifferential);

      expect(nightShiftPay).toBe(5000);
    });
  });
  ...
  describe('Deductions', () => {
    ...
    it('should calculate loan installment deduction', () => {
      ...
      const months = 12;
      const monthlyInstallment = loanAmount / months;
