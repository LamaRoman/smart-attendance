...
export default function MyInfoPage() {
  ...
  const savePassword = async () => {
    ...
    if (!pwForm.currentPassword) {
      setPwMsg({
        type: 'error',
        text: isNp ? 'हालको पासवर्ड आवश्यक छ' : 'Current password is required',
      })
      return
    }
    if (!pwForm.newPassword) {
      setPwMsg({ type: 'error', text: isNp ? 'नयाँ पासवर्ड आवश्यक छ' : 'New password is required' })
      return
    }
    if (pwForm.newPassword !== pwForm.confirmPassword) {
      setPwMsg({ type: 'error', text: isNp ? 'पासवर्ड मेल खाएन' : 'Passwords do not match' })
      return
    }
    ...
    if (!/[A-Z]/.test(pw)) {
      setPwMsg({ type: 'error', text: 'Must contain at least one uppercase letter' })
      return
    }
    if (!/[a-z]/.test(pw)) {
      setPwMsg({ type: 'error', text: 'Must contain at least one lowercase letter' })
      return
    }
    if (!/[0-9]/.test(pw)) {
      setPwMsg({ type: 'error', text: 'Must contain at least one number' })
      return
    }
    if (!/[!@#$%^&*(),.?":{}|<>]/.test(pw)) {
      setPwMsg({ type: 'error', text: 'Must contain at least one special character' })
      return
    }
    ...
    try {
      ...
      setPwMsg({
        type: 'success',
        text: isNp ? 'पासवर्ड सफलतापूर्वक परिवर्तन भयो!' : 'Password changed successfully!',
      })
    } catch (e: any) {
  ...
  const savePin = async () => {
    ...
    if (!pinForm.currentPin || pinForm.currentPin.length !== 4) {
      setPinMsg({
        type: 'error',
        text: isNp ? 'हालको PIN ४ अंकको हुनुपर्छ' : 'Current PIN must be 4 digits',
      })
      return
    }
    if (!pinForm.newPin || pinForm.newPin.length !== 4) {
      setPinMsg({
        type: 'error',
        text: isNp ? 'नयाँ PIN ४ अंकको हुनुपर्छ' : 'New PIN must be 4 digits',
      })
      return
    }
    if (pinForm.newPin !== pinForm.confirmPin) {
      setPinMsg({ type: 'error', text: isNp ? 'PIN मेल खाएन' : 'PINs do not match' })
      return
    }
    if (pinForm.currentPin === pinForm.newPin) {
      setPinMsg({
        type: 'error',
        text: isNp
          ? 'नयाँ PIN हालकोभन्दा फरक हुनुपर्छ'
    ...
    setPinSaving(true)
    try {
      const res = await api.patch('/api/v1/auth/attendance-pin', {
      ...
      setPinMsg({
        type: 'success',
        text: isNp ? 'PIN सफलतापूर्वक परिवर्तन भयो!' : 'Attendance PIN changed successfully!',
      })
    } catch (e: any) {
  ...
  const handleForgotPin = async () => {
    if (
      !confirm(
        isNp
          ? 'नयाँ PIN तपाईंको इमेलमा पठाइनेछ। जारी राख्ने?'
    ...
    try {
      ...
      setPinMsg({
        type: 'success',
        text: isNp ? 'नयाँ PIN तपाईंको इमेलमा पठाइयो।' : 'A new PIN has been sent to your email.',
      })
    } catch (e: any) {
  ...
  const inputClass =
    'w-full pl-9 pr-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-300 focus:border-slate-400 transition-colors bg-white'
  const pinInputClass =
    'w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-300 focus:border-slate-400 transition-colors bg-white text-center tracking-widest font-medium'
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50">
      ...
      <div className="mx-auto w-full max-w-3xl flex-1 space-y-5 px-5 py-6">
        {/* Identity card */}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                <th className="w-1/2 px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-400">
                  {isNp ? 'विवरण' : 'Field'}
                </th>
                <th className="w-1/2 px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-400">
                  {isNp ? 'मूल्य' : 'Value'}
              ...
              <tr className="transition-colors hover:bg-slate-50/50">
                ...
                <td className="px-5 py-3 text-sm font-medium text-slate-900">
                  {(user as any).createdAt
                    ? calendarMode === 'NEPALI'
                      ? (() => {
                          const bs = adToBS(new Date((user as any).createdAt))
          ...
          <div className="space-y-4 p-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={isNp ? 'पहिलो नाम' : 'First name'}
                  />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={
                      isNp
                        ? 'आफ्नो हालको पासवर्ड प्रविष्ट गर्नुहोस्'
            ...
            <div className="flex items-center gap-2 pt-1">
              <button
                ...
                className="flex items-center gap-1.5 rounded-lg bg-slate-800 px-4 py-2 text-xs font-medium text-white transition-colors hover:bg-slate-900 disabled:cursor-not-allowed disabled:opacity-40"
              >
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          ...
          <div className="space-y-4 p-5">
            ...
            <button
              ...
              className="text-xs font-medium text-slate-500 transition-colors hover:text-slate-700 disabled:opacity-40"
            >
              {resettingPin
                ? isNp
                  ? 'पठाउँदै...'
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          ...
          <div className="space-y-4 p-5">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-200 bg-white py-2 pl-9 pr-10 text-sm transition-colors focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-300"
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-slate-500">
                {isNp ? 'नयाँ पासवर्ड' : 'New Password'}
              </label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" />
                <input
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-slate-500">
                {isNp ? 'पासवर्ड पुष्टि गर्नुहोस्' : 'Confirm New Password'}
              </label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" />
                <input
