...
import { ArrowLeft, Save, Plus, Trash2, AlertCircle, CheckCircle, RefreshCw, Calculator } from 'lucide-react';
...
useEffect(() => {
  ...
  const calculateTest = () => {
    ...
    const allSlabs = [
      { limit: firstSlabLimit, rate: config.firstSlabRate / 100, label: `First slab (${config.firstSlabRate}%)` },
      ...config.slabs.map((s) => ({
    ...
    for (const slab of allSlabs) {
      ...
      tax += slabTax;
      breakdown.push({ label: slab.label, taxable, rate: slab.rate * 100, tax: Math.round(slabTax) });
      remaining -= taxable;
    }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center gap-3">
              <button onClick={() => router.push('/super-admin')} className="p-1.5 hover:bg-slate-100 rounded-md transition-colors">
            ...
            <div className="flex items-center gap-2">
              <button onClick={loadSlabs} disabled={loading}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-lg disabled:opacity-50">
                <RefreshCw className={'w-3.5 h-3.5 ' + (loading ? 'animate-spin' : '')} /> Reload
              </button>
              <button onClick={handleSave} disabled={saving}
                className="flex items-center gap-1.5 px-4 py-1.5 text-xs font-medium text-white bg-slate-900 hover:bg-slate-800 rounded-lg disabled:opacity-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {error && (
          <div className="flex items-center justify-between p-3 bg-rose-50 rounded-lg border border-rose-200">
            ...
            <button onClick={() => setError('')} className="text-rose-400 text-xs">✕</button>
          </div>
        )}
        {success && (
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
          <p className="text-xs text-amber-800">
            These tax slabs apply to <strong>all organizations</strong> on the platform. Changes take effect on the next payroll generation.
            {config.updatedAt && (
              <span className="ml-2 text-amber-600">Last updated: {new Date(config.updatedAt).toLocaleDateString()}</span>
            )}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <h2 className="text-sm font-semibold text-slate-900 mb-4">General settings</h2>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Fiscal Year (BS)</label>
                  <input type="text" value={config.fiscalYear}
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Unmarried 1st slab (Rs.)</label>
                  <input type="number" value={config.unmarriedFirstSlab}
                    onChange={(e) => setConfig({ ...config, unmarriedFirstSlab: Number(e.target.value) })}
                    className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-200" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Married 1st slab (Rs.)</label>
                  <input type="number" value={config.marriedFirstSlab}
                    onChange={(e) => setConfig({ ...config, marriedFirstSlab: Number(e.target.value) })}
                    className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-200" />
                </div>
              </div>
              <div className="mt-4">
                <label className="block text-xs font-medium text-slate-500 mb-1">First slab tax rate (%)</label>
                <input type="number" value={config.firstSlabRate} step="0.1"
                  onChange={(e) => setConfig({ ...config, firstSlabRate: Number(e.target.value) })}
                  className="w-32 px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-200" />
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-sm font-semibold text-slate-900">Tax slabs (after first slab)</h2>
                  <p className="text-xs text-slate-400 mt-0.5">Slab widths apply equally to both married and unmarried — only the starting threshold differs</p>
                </div>
                <button onClick={addSlab}
                  className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-lg">
              ...
              <div className="space-y-3">
                {/* Header */}
                <div className="grid grid-cols-12 gap-3 text-xs font-medium text-slate-400 uppercase tracking-wider px-1">
                  ...
                  return (
                    <div key={idx} className="grid grid-cols-12 gap-3 items-center">
                      <div className="col-span-4">
                        <input type="text" value={slab.label}
                          onChange={(e) => updateSlab(idx, 'label', e.target.value)}
                          className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-200" />
                      </div>
                      <div className="col-span-3">
                        {isLast ? (
                          <div className="px-3 py-2 text-sm text-slate-400 bg-slate-50 rounded-lg border border-slate-200">Remaining</div>
                        ) : (
                          <input type="number" value={slab.limit}
                            onChange={(e) => updateSlab(idx, 'limit', Number(e.target.value))}
                            className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-200" />
                        )}
                      </div>
                      <div className="col-span-3">
                        <input type="number" value={slab.rate} step="0.1"
                          onChange={(e) => updateSlab(idx, 'rate', Number(e.target.value))}
                          className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-200" />
                      </div>
                      <div className="col-span-2 flex justify-end">
                        {!isLast && config.slabs.length > 1 && (
                          <button onClick={() => removeSlab(idx)}
              ...
              <div className="mt-5 pt-4 border-t border-slate-100">
                <div className="grid grid-cols-2 gap-4">
                  {/* Unmarried preview */}
                  <div>
                    <h3 className="text-xs font-semibold text-slate-700 mb-2">Unmarried brackets</h3>
                    <div className="space-y-0.5">
                      {unmarriedRanges.map((r, i) => (
                        <div key={i} className="flex items-center justify-between text-xs py-1 px-2 rounded hover:bg-slate-50">
                  ...
                  {/* Married preview */}
                  <div>
                    <h3 className="text-xs font-semibold text-slate-700 mb-2">Married brackets</h3>
                    <div className="space-y-0.5">
          ...
          <div>
            <div className="bg-white rounded-xl border border-slate-200 p-5 sticky top-20">
              ...
              <p className="text-xs text-slate-400 mb-4">Test with current slab configuration</p>

              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Annual income (Rs.)</label>
                  <input type="number" value={testIncome}
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={testMarried}
                ...
                <button onClick={calculateTest}
                  className="w-full py-2 bg-slate-900 text-white rounded-lg text-xs font-medium hover:bg-slate-800">
                  Calculate
                </button>
                  ...
                  <div className="mt-3 space-y-3">
                    ...
                    <div className="p-3 bg-slate-50 rounded-lg space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-semibold text-slate-900">{formatNPR(testResult.annual)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-xs text-slate-500">Monthly TDS</span>
                        <span className="text-sm font-semibold text-emerald-700">{formatNPR(testResult.monthly)}</span>
                      </div>
                      <div className="flex justify-between">
                    ...
                    <div className="p-3 bg-blue-50/50 rounded-lg border border-blue-100">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="pt-1.5 mt-1.5 border-t border-blue-200 flex justify-between text-xs font-semibold">
