...
function post(path, data, cookie) {
  return new Promise((resolve, reject) => {
    ...
    const opts = {
      ...
      headers: { 'Content-Type': 'application/json', 'Content-Length': body.length, 'X-Requested-With': 'XMLHttpRequest' }
    };
    if (cookie) opts.headers.Cookie = cookie;
    const req = http.request(opts, res => { let d = ''; res.on('data', c => d += c); res.on('end', () => resolve({ status: res.statusCode, headers: res.headers, body: JSON.parse(d) })); });
    req.on('error', reject); req.write(body); req.end();
  });
}
async function test() {
  const login = await post('/api/auth/login', { email: 'sita@democompany.com', password: 'Employee@123' });
  if (login.status !== 200) { console.log('Login failed:', login.body); return; }
  const cookie = login.headers['set-cookie']?.[0]?.split(';')[0];
  console.log('Logged in as Sita');
  console.log('\n--- Test 1: Outside geofence (~89km away) ---');
